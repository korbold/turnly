...
class ReservationRepositoryImpl implements IReservationRepository {
  ...
  Future<Either<Failure, List<Reservation>>> getAll({String? status}) async {
    try {
      ...
      developer.log('RAW RESPONSE: ${response.data}', name: 'ReservationRepo');
      final data = response.data['data'] as List<dynamic>;
      final reservations = <Reservation>[];
      for (var i = 0; i < data.length; i++) {
        final json = data[i] as Map<String, dynamic>;
        developer.log('ITEM $i: $json', name: 'ReservationRepo');
        try {
          reservations.add(ReservationDto.fromJson(json).toEntity());
        } catch (e, st) {
          developer.log('PARSE ERROR item $i: $e\n$st', name: 'ReservationRepo');
          rethrow;
        }
      }

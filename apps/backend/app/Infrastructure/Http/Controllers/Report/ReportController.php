    ...
    public function daily(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'data' => [
                'date' => $date,
                'washes' => [
                    'total' => $washLogs->count(),
                    'completed' => $washLogs->where('status', 'completed')->count(),
                    'in_progress' => $washLogs->where('status', 'in_progress')->count(),
                    'revenue' => $washLogs->sum('price_charged'),
                    'by_payment_method' => [
    ...
    public function weekly(Request $request): JsonResponse
    {
        ...
        for ($i = 0; $i < 7; $i++) {
            ...
            $dailyBreakdown[$dayStr] = [
                'washes' => $dayLogs->count(),
                'revenue' => (float) $dayLogs->sum('price_charged'),
            ];
        }
        ...
        return response()->json([
            'data' => [
                'week' => $week,
                'total_washes' => $washLogs->count(),
                'total_revenue' => (float) $washLogs->sum('price_charged'),
    ...
    public function monthly(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'data' => [
                ...
                'average_daily_washes' => $washLogs->count() > 0
                    ? round($washLogs->count() / $washLogs->groupBy(fn($l) => $l->log_date->format('Y-m-d'))->count(), 1)
                    : 0,
            ],
            'meta' => [
                'tenant' => app('current_tenant')->slug ?? null,
